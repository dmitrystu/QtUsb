...
#define LibusbSetLoglevel(ctx, loglevel) libusb_set_option(ctx, LIBUSB_OPTION_LOG_LEVEL, loglevel)
#else
#define LibusbSetLoglevel(ctx, loglevel) libusb_set_debug(ctx, loglevel)
#endif
    ...
    : QObject(*(new QUsbPrivate), parent), d_dummy(Q_NULLPTR)
{
    ...
    LibusbSetLoglevel(d->m_ctx, LIBUSB_LOG_LEVEL_WARNING);
...
QUsb::IdList QUsb::devices()
{
    ...
    libusb_init(&ctx);
    LibusbSetLoglevel(ctx, LIBUSB_LOG_LEVEL_NONE);
...
void QUsb::setLogLevel(QUsb::LogLevel level)
{
    ...
    m_log_level = level;
    if (m_log_level >= QUsb::logDebug)
        LibusbSetLoglevel(d->m_ctx, LIBUSB_LOG_LEVEL_DEBUG);
    else if (m_log_level >= QUsb::logWarning)
        LibusbSetLoglevel(d->m_ctx, LIBUSB_LOG_LEVEL_WARNING);
    else
        LibusbSetLoglevel(d->m_ctx, LIBUSB_LOG_LEVEL_ERROR);
}
